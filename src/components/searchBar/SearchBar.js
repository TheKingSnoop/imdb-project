...
const SearchBar = ({ API_HOST, setMovies, isDarkMode, getTop20Movies, setLandingPageText }) => {
  ...
  return (
    <Box sx={{ margin:"auto", maxWidth: "1240px", padding: '30px 30px 0px 30px', marginBottom:'20px'}}>
      <form onSubmit={handleSubmit}>
        <Grid container spacing={2} sx={{ display: 'flex', justifyContent: 'center' }}>
          ...
          <Grid item md={3} sm={3} xs={5}><Button variant='contained' fullWidth sx={{ height: '100%', backgroundColor: 'primary.light' }} type="submit">Search</Button>
          </Grid>
          <Grid item md={3} sm={3} xs={12}><Button variant='contained' fullWidth sx={{height: {sm:'100%', xs:'auto'} ,backgroundColor: 'primary.light' }} onClick={handleClickViewAll}>View trending movies</Button>
          </Grid>
